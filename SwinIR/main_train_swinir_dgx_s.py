...
def main():
    ...
    parser.add_argument('--tag', type=str, default="MR2CT_S_", help='Save_prefix')
    ...
    parser.add_argument('--folder_pet', type=str, default="./MR2CT_S/X/train/", help='input folder of T1MAP images')
    parser.add_argument('--folder_sct', type=str, default="./MR2CT_S/Y/train/", help='input folder of BRAVO images')
    parser.add_argument('--folder_pet_v', type=str, default="./MR2CT_S/X/val/", help='input folder of T1MAP PET images')
    parser.add_argument('--folder_sct_v', type=str, default="./MR2CT_S/Y/val/", help='input folder of BRAVO images')
    ...
    for idx_epoch in range(args.epoch):
        ...
        for cnt_sct, sct_path in enumerate(sct_list):
            ...
            np.save(args.tag+"Epoch[{:03d}]_Case[{}]_t.npy".format(idx_epoch+1, case_name), (batch_x.cpu().numpy(), batch_y.cpu().numpy(), model(batch_x).cpu().numpy()))
        ...
        for cnt_sct, sct_path in enumerate(sct_list_v):
            ...
            case_name = os.path.basename(cube_x_path)[4:7]
            np.save(args.tag+"Epoch[{:03d}]_Case[{}]_v.npy".format(idx_epoch+1, case_name), (batch_x.cpu().numpy(), batch_y.cpu().numpy(), model(batch_x).cpu().numpy()))
